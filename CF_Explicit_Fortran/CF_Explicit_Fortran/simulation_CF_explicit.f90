program simulation
    ...
    do while (N_cycles<=number_cycles .and. step_in_cycle<n_step)
        ...
        if (stadia==2) then
            ...
            call OneStep(eps_cr, eps_p, eps_ii1, eps_ii2, eps_ii3, sigma, f, omega, eps, neps_cr, neps_p, neps_ii1, neps_ii2, neps_ii3, nomega, nf, k, mu, c1, c2, c3, kappa1, kappa2, kappa3,eta1, eta2, eta3, n_Norton, A_Norton, n_x, K_yield, D, sigma1, eta, A_nuc, B, m, dt_creep)
            eps(1,1)=eps(1,1)-(sigma(1,1)-stress_current)*damping
            eps(2,2)=eps(2,2)-sigma(2,2)*damping
            eps(3,3)=eps(2,2)
            if(abs((sigma(1,1)-stress_current)*damping)>5.0d-6) then
                dt_creep=dt_creep*0.8d0
            else 
                dt_creep=min(dt_creep*1.2d0, 0.1d0)
            end if
            if (n==590) then
                continue
            end if
            if (eps(1,1)>=strain_max) then
              ...
              creep_rate(N_cycles)=abs(eps_cr_end-eps_cr_start)/(t_cr_end-t_cr_start)
            end if
        end if
        if (stadia==3) then
            time=time+dt_ep
